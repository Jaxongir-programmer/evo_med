...
class FPConfirmOTPScreen extends StatefulWidget {
  const FPConfirmOTPScreen({super.key});

  @override
  State<FPConfirmOTPScreen> createState() => _FPConfirmOTPScreenState();
}

class _FPConfirmOTPScreenState extends State<FPConfirmOTPScreen> {
  late Timer _timer;
                  ...
                  if (value.length == 4) {
                    context.read<AuthBloc>().add(
                          CheckVerificationCodeEvent(
                            code: value.toInt(),
                            onSuccess: () {
                              context.goNamed(AppRouter.setNewPassword);
                            },
                            onError: (error) {
